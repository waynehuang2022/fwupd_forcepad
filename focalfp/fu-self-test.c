...
static void
fu_focalfp_firmware_xml_func(void)
{
	gboolean ret;
	...
	g_autoptr(FuFirmware) firmware1 = fu_focalfp_firmware_new();
	g_autoptr(FuFirmware) firmware2 = fu_focalfp_firmware_new();
	g_autoptr(GError) error = NULL;

	/* build and write */
	filename = g_test_build_filename(G_TEST_DIST, "tests", "focalfp.builder.xml", NULL);
...
main(int argc, char **argv)
{
	...
	/* tests go here */
	g_test_add_func("/focalfp/firmware{xml}", fu_focalfp_firmware_xml_func);
	return g_test_run();
}
